...
class CarpollMapController extends GetxController implements GetxService {
  ...
  Future<void> getPolyline() async {
    if (Get.find<CarPollRideController>().encodedPolyLine.isNotEmpty) {
      List<LatLng> polylineCoordinates = [];
      List<LatLng> result = decodeEncodedPolyline(
          Get.find<CarPollRideController>().encodedPolyLine);
  ...
  Future<void> searchDeliveryMen() async {
    ...
    for (int i = 0;
        i < Get.find<CarPollRideController>().nearestDriverList.length;
        i++) {
      ...
      nearestDeliveryManMarkers!.add(Marker(
        ...
        position: LatLng(
            double.parse(Get.find<CarPollRideController>()
                .nearestDriverList[i]
                .latitude!),
            double.parse(Get.find<CarPollRideController>()
                .nearestDriverList[i]
        ...
        icon: BitmapDescriptor.fromBytes(
            Get.find<CarPollRideController>().nearestDriverList[i].category ==
                    'motor_bike'
  ...
  void setFromToMarker(LatLng from, LatLng to,
    ...
    markers = HashSet();
    Uint8List fromMarker = await convertAssetToUnit8List(
      Images.fromIcon,
      width: 125,
      blendColor: Colors.black,
      blendMode: BlendMode.srcATop,
    );
    Uint8List toMarker = await convertAssetToUnit8List(
      Images.targetLocationIcon,
      width: 100,
      blendColor: Colors.black,
      blendMode: BlendMode.srcATop,
    );
    ...
    markers.add(Marker(
      ...
      infoWindow: InfoWindow(
        title:
            Get.find<CarPollRideController>().tripDetails?.pickupAddress ?? '',
    ...
    markers.add(Marker(
      ...
      infoWindow: InfoWindow(
        title:
            Get.find<CarPollRideController>().tripDetails?.destinationAddress ??
                '',
  ...
  Future<Uint8List> convertAssetToUnit8List(String imagePath,
      {int width = 50, Color? blendColor, BlendMode? blendMode}) async {
    ...
    if (blendColor != null && blendMode != null) {
      // Create a picture recorder to apply blend mode
      ...
      // Apply blend mode with the specified color
      Paint blendPaint = Paint()
        ..color = blendColor
        ..blendMode = blendMode;
      ...
      ui.Image blendedImage =
          await picture.toImage(fi.image.width, fi.image.height);

      return (await blendedImage.toByteData(format: ui.ImageByteFormat.png))!
          .buffer
          .asUint8List();
    }

    // Return original image if no blend mode specified
  ...
  void updateDriverMarker(List<LatLng> latLngList) async {
    ...
    Uint8List car = await convertAssetToUnit8List(
      Get.find<CarPollRideController>().tripDetails!.vehicleCategory!.type ==
              'car'
          ? Images.carTop
          : Images.bike,
      width: 55,
  ...
  void setMarkersInitialPosition() {
    if (Get.find<CarPollRideController>().encodedPolyLine.isNotEmpty) {
      List<LatLng> markers = decodeEncodedPolyline(
          Get.find<CarPollRideController>().encodedPolyLine);
      setFromToMarker(
