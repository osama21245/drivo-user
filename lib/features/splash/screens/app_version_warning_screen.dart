        ...
        body: Center(
          child: Padding(
            ...
            child: Column(children: [
              ...
              ButtonWidget(
                buttonText:
                    '${'download_new_version'.tr} (${Platform.isAndroid ? Get.find<ConfigController>().config?.androidAppMinimumVersion : Get.find<ConfigController>().config?.iosAppMinimumVersion})',
                onPressed: () {
                  if (Platform.isAndroid) {
                    launchUrl(Uri.parse(
                        Get.find<ConfigController>().config?.androidAppUrl ??
                            ''));
                  } else if (Platform.isIOS) {
                    launchUrl(Uri.parse(
                        Get.find<ConfigController>().config?.iosAppUrl ?? ''));
                  }
                },
              ),
            ]),
